...
EBTNodeResult::Type UBTTask_MoveToPlayer::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	AEnemyMaster* Enemy = Cast<AEnemyMaster>(EnemyPC->GetPawn()); // get the enemy pawn instance

	AEndlessReachHDPawn* Player = Cast<AEndlessReachHDPawn>(OwnerComp.GetBlackboardComponent()->GetValue<UBlackboardKeyType_Object>(EnemyPC->PlayerKeyID)); // Get target from blackboard
	
	if (Player)  // make sure player reference is valid
	{
		if (Enemy)  // make sure enemy reference is valid
		{
			if (Enemy->bRunAI)  // Make sure enemy has been aggroed by the player
			{
